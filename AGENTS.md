...
## 结构原则：拆成小实验，不要做成一个大项目

这条线覆盖五个差异极大的岗位方向。**做一个「什么都沾一点」的大项目，结果是五个方向哪个都覆盖不了** —— 这是本项目最大的风险。

所以拆成各自独立、各自能讲完整故事的小实验：

| 岗位 | 实验 | 核心考点 |
|---|---|---|
| 06 字节 后端（导购） | 导购服务 + 缓存一致性 | 数据一致性、接口设计、降级 |
| 13 米哈游 游戏服务器 | 房间状态同步服务 | 长连接、状态同步、帧驱动 |
| 15 拼多多 C++ 研发 | 规则表达式求值引擎 | AST／字节码、性能、benchmark |
| 16 拼多多 弹性计算 | 简化版调度器（打分、装箱、抢占） | 资源调度算法 |
| 17 拼多多 云原生（CMDB） | 配置项模型 + 关系图 + 变更审计 | 数据建模、图查询、变更管理 |

## 覆盖边界

实验共用基础组件（容器化、可观测、测试方法）能省时间，但**共用组件不等于五个方向的专项能力都已覆盖**。每个实验的实际覆盖范围在各自目录的 README 里说明，**不做笼统声称**。
...
tx 服务器适合跑这条线。［实测 2026-09-20］go1.27.1 / g++ 13.3 / cmake 3.28 齐全；`go get` 走腾讯云镜像直连可达，**不需要代理**；`redis:7-alpine` 镜像已备好。

每个实验是**独立的 Go module**，放在 `experiments/<eN-名字>/` 下，不做 workspace 聚合 —— 五个实验技术栈不同（其中 E3 是 C++），强行统一构建只会互相牵制。内存只有 7.5G，多个服务同时跑之前先看 `free -h`。
